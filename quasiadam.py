...
__all__ = ['quasiAdam', 'quasiadam']


class quasiAdam(Optimizer):
    ...
    def _init_group(
        ...
        for p in group['params']:
            if p.grad is not None:
                ...
                if p.grad.is_sparse:
                    raise RuntimeError('quasiAdam does not support sparse gradients, please consider SparsequasiAdam instead')
                grads.append(p.grad)
                state = self.state[p]
                ...
                if len(state) == 0:
                    ...
                    state['prev_params'] = torch.zeros_like(p, memory_format=torch.preserve_format)
                ...
                prev_params.append(state['prev_params'])
...
quasiAdam.__doc__ = r"""Implements quasiAdam algorithm.
    ...
    For further details regarding the algorithm we refer to `quasiAdam: A Method for Stochastic Optimization`_.
    """ + r"""
    Args:
        ...
        amsgrad (bool, optional): whether to use the AMSGrad variant of this
            algorithm from the paper `On the Convergence of quasiAdam and Beyond`_
            (default: False)
        {foreach}
        {maximize}
        {capturable}
        {differentiable}
        {fused}
    .. _quasiAdam\: A Method for Stochastic Optimization:
        https://arxiv.org/abs/1412.6980
    .. _On the Convergence of quasiAdam and Beyond:
...
def quasiadam(params: List[Tensor],
         grads: List[Tensor],
         ...
         *,
         ...
         maximize: bool,
         first: bool):
    r"""Functional API that performs quasiAdam algorithm computation.
    See :class:`~torch.optim.quasiAdam` for details.
    """
    ...
    func(params,
         ...
         found_inf=found_inf,
         prev_grads=prev_grad,
         prev_params=prev_params,
         first=first)


def _single_tensor_quasiadam(params: List[Tensor],
                        grads: List[Tensor],
                        exp_avgs: List[Tensor],
                        exp_avg_sqs: List[Tensor],
                        max_exp_avg_sqs: List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        # Compute the step S
        # Compute change in grad Y
        # compute s.dot(s)/y.dot(s)
