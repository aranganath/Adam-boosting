...
class ARCLSR1(Optimizer):
	...
	def __init__(self, 
		...
		super(ARCLSR1, self).__init__(params, defaults)
		...
		self.param_groups[0]['exp_avg'] = torch.zeros(sum([p.numel() for p in self._params]), device=self._params[0].device.type)
	...
	def step(self, closure=None):
		...
		exp_avg = self.param_groups[0]['exp_avg']
		orig_loss = closure()
		...
		while n_iters < max_iters:
			...
			if state['n_iters'] == 1:
				...
				D, g_parallel, C_parallel, U_par, alphastar,sstar, gamma, pflag = self.LSR1(S, SS, YY, SY, Y, flat_grad, gamma, mu)
			...
			if line_search_fn is not None:
				...
				if flag:
					...
					# Do the exp moving average
					set_trace()
					exp_avg.mul_(1-beta1).add_(sstar, alpha=beta1)
					exp_avg_sq = state['exp_avg']
					exp_avg_sq.mul_(1-beta2).addcmul_(sstar, sstar.conj(), value=beta2)

					bias_correction1 = 1 - torch.pow(beta1, n_iters)
					bias_correction2 = 1 - torch.pow(beta2, n_iters)
	...
	def LSR1(self, S, SS, YY, SY, Y, flat_grad, gammaIn, mu):
		...
		sstar = -alphastar * flat_grad + U_par @ (alphastar * torch.eye(S.shape[1], device=device) - C_parallel) @ g_parallel
